...
export async function adminView(outlet) {
  const tabs = [
    ...
    { id: 'occupancies',label: 'Ocupación',     perm: 'admin.catalog' },
  ...
  const show = async (id) => {
    ...
    if (id === 'statuses') {
      return catalogTab(box, {
        ...
        fields: () => [
          ...
          { name: 'requires_vacant', label: 'Exige habitación sin huésped', type: 'checkbox',
            hint: 'Márquelo sólo en el estado de venta: una habitación ocupada no podrá quedar en él.' },
    ...
    if (id === 'occupancies') {
      return catalogTab(box, {
        path: 'occupancies', title: 'Ocupación de la habitación',
        newLabel: 'Nueva ocupación', entityName: 'Ocupación',
        subtitle: 'Eje aparte del estado: dice si hay huésped, no en qué punto de la limpieza va.',
        columns: [
          { header: 'Ocupación', render: (o) => `<span class="row" style="gap:7px;color:${esc(o.color)}">${icon(o.icon, 15)}<strong>${esc(o.name)}</strong></span>` },
          { key: 'code', header: 'Código' },
          { header: 'Significa', render: (o) => [
            o.counts_occupied && 'Hay huésped', o.do_not_disturb && 'No molestar', o.is_default && 'Sin huésped (por omisión)',
          ].filter(Boolean).map((x) => `<span class="chip tiny">${x}</span>`).join(' ') || '<span class="muted tiny">—</span>' },
          { header: 'Activa', render: (o) => activeChip(o.active) }],
        fields: () => [
          { name: 'code', label: 'Código', required: true, placeholder: 'LLEGADA_HOY' },
          { name: 'name', label: 'Nombre visible', required: true },
          { name: 'icon', label: 'Icono', type: 'select', options: ['user', 'door', 'logout', 'ban', 'lock',
            'clock', 'bell', 'alert'].map((i) => ({ value: i, label: i })) },
          { name: 'color', label: 'Color', type: 'text', placeholder: '#2563eb' },
          { name: 'description', label: 'Descripción', type: 'textarea' },
          { name: 'counts_occupied', label: 'Hay huésped en la habitación', type: 'checkbox',
            hint: 'Con esto marcado, la habitación no puede pasar a un estado de venta.' },
          { name: 'do_not_disturb', label: 'No se entra a la habitación', type: 'checkbox' },
          { name: 'is_default', label: 'Es la ocupación "sin huésped"', type: 'checkbox',
            hint: 'A ella vuelve la habitación cuando se pone de nuevo a la venta.' },
          { name: 'target_status_id', label: 'Estado al entrar el huésped', type: 'select', allowEmpty: true,
            options: state.statuses.map((s) => ({ value: s.id, label: s.name })),
            hint: 'Estado al que pasa una habitación a la venta cuando se registra esta ocupación.' },
          { name: 'sort_order', label: 'Orden', type: 'number' },
    ...
    if (id === 'actions') {
      return catalogTab(box, {
        ...
        fields: () => [
          ...
          { name: 'target_occupancy_id', label: 'Ocupación destino', type: 'select', allowEmpty: true,
            options: state.occupancies.map((o) => ({ value: o.id, label: o.name })),
            hint: 'Para entradas y salidas de huésped. Déjelo vacío si la acción no la cambia.' },
          ...
          { name: 'closes_scope', label: 'Alcance del cierre', type: 'select',
            options: [{ value: 'categoria', label: 'Sólo lo pendiente de su área' },
                      { value: 'habitacion', label: 'Todo lo pendiente de la habitación' }],
            hint: 'Sólo aplica si la acción cierra incidencias.' },
