    ...
    if (id === 'statuses') {
      return catalogTab(box, {
        ...
        fields: () => [
          ...
          { name: 'clean_status_id', label: 'Estado al terminar la limpieza', type: 'select', allowEmpty: true,
            options: state.statuses.map((s) => ({ value: s.id, label: s.name })),
            hint: 'A dónde pasa una habitación en este estado cuando se termina de limpiarla.' },
    ...
    if (id === 'actions') {
      return catalogTab(box, {
        ...
        fields: () => [
          ...
          { name: 'target_from_clean', label: 'El destino lo decide el estado actual', type: 'checkbox',
            hint: 'Para "Limpieza terminada": lleva al estado que cada estado declare al terminar la limpieza.' },
          { name: 'asks_guest_present', label: 'Pregunta si el huésped estará', type: 'checkbox',
            hint: 'Para los reportes: el área que sube necesita saber qué se va a encontrar.' },
